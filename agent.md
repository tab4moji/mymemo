...
```markdown:コミットプッシュ依頼
git に現在のプロジェクトファイル差分を記録したい。

git add . したあとに git diff と git diff --cached で差分を確認して、差分があればコミットメッ セージを考えて日本語でコミットとプッシュしてくれ。
ccc search "<処理内容>" などで、関数名が分かる場合は、どのファイルのどの関数を変更したかも記してくれ。
