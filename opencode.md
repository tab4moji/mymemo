...
```markdown:コード分析
- このプロジェクトを解析したい。そのために、まずドキュメントファイルとソースコードファイルに何があるか一覧を探して、./.works/docs_and_codes_<現在時刻>.md というファイル名に記録すること本文中にも現在時刻を記録すること。そのとき、決してファイルを描き込んだり、コードを実行しないこと。
```
