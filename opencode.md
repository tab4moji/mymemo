## OpenCode

### 概要

エージェントコーディング CLI ツール。
勝手なアクセスが不安なら、[ip netns / iptables とかを使いましょう](./?content=isolate)。

### 公式サイト

- https://opencode.ai/
- https://github.com/anomalyco/opencode

### 噂

- サーバー側の ctx サイズは、32 Ki 以上じゃないと使い物にならない
- お仕事の種類によって接続先を変更できるらしい
  - https://opencode.ai/docs/ja/agents/#json
- [cocoindex-code](./?content=cocoindex-code) がいいらしい

### 推論エンジン

- [LiteRT-LM](./?content=litertlm) で [Gemma4-26B-AB](https://huggingface.co/litert-community/gemma-4-26B-A4B-it-litert-lm) を使ってます

### インストール

```bash
if [[ ! $(which opencode) ]]; then \curl -fsSL https://opencode.ai/install | bash && mkdir -p ~/.config/opencode/; fi
```

### アンインストール

```bash
if [[ $(which opencode) ]]; then opencode uninstall && { rm -rf ~/.cache/opencode; rm -rf ~/.config/opencode; rm -rf ~/.opencode; }; fi
```

### opencode.json 設定

```json:~/.config/opencode/opencode.json
{
  "$schema": "https://opencode.ai/config.json",
  "model": "ollama_host/gemma4-26b-a4b",
  "provider": {
    "ollama_host": {
      "npm": "@ai-sdk/openai-compatible",
      "name": "Ollama on Internal Server",
      "options": {
        "baseURL": "http://192.168.0.11:11434/v1"
      },
      "models": {
        "gemma4-26b-a4b": {
          "name": "gemma4-26b-a4b",
          "max_tokens": 32768,
          "tools": true
        }
      }
    }
  },
  "enabled_providers": ["ollama_host"],
  "autoupdate": false,
  "share": "disabled",
  "permission": {
    "websearch": "deny",
    "webfetch": "deny",
    "share": "deny",
    "read": {
      "*.env*": "deny",
      "*.pem": "deny"
    }
  },
  "experimental": {
    "openTelemetry": false
  },
  "mcp": {
    "cocoindex-code": {
      "type": "local",
      "command": [
        "uvx",
        "--prerelease=explicit",
        "--with",
        "cocoindex>=1.0.0a16",
        "cocoindex-code@latest"
      ]
    }
  }
}
```

### 起動時の注意

2026年時点のOpenCodeの挙動に関する開発コミュニティの報告によると、設定ファイルの指定だけでは、モデルリストの取得（models.devへのアクセス）などで微小な外部通信が発生する場合がある。
これを完全に防ぎ、社内ネットワーク内に閉じ込めるには、OpenCodeの起動時に以下の環境変数を併用するのが有効だ。

- 勝手なアクセスが不安なら、[ip netns / iptables とかを使いましょう](./?content=isolate)

```bash
export OPENCODE_DISABLE_AUTOUPDATE=true
export OPENCODE_DISABLE_SHARE=true
export OPENCODE_DISABLE_MODELS_FETCH=true
```

これらを `~/.bashrc` や `~/.zshrc` に記述しておくことで、意図しない外部へのデータ流出をより強固に防ぐことができる。

### opencode と ollama の会話をチラ見

```bash:opencodeデバッグ
DB=~/.local/share/opencode/opencode.db
LAST_FILE=/tmp/opencode_last_ts

sqlite3 "$DB" "SELECT COALESCE(max(time_created), 0) FROM part;" < /dev/null > "$LAST_FILE"
echo "監視開始 (last=$(cat $LAST_FILE))"

inotifywait -m -e modify "${DB}-wal" 2>/dev/null | while IFS= read -r _; do
  LAST=$(cat "$LAST_FILE")

  sqlite3 "$DB" \
    "SELECT time_created, json_extract(data, '$.type'), substr(data, 1, 120)
     FROM part
     WHERE time_created > ${LAST}
     ORDER BY time_created ASC;" < /dev/null \
  | while IFS='|' read -r ts type data; do
      TIME=$(date -d "@$((ts / 1000))" '+%H:%M:%S')
      echo "[$TIME][$type] $data"
      echo "$ts" > "$LAST_FILE"
    done

done
```

### 使い方メモ

- [使い方メモ](./?content=agent)

##

OpenCode における Plan と Build は作業フェーズを分ける主幹エージェントであり、Execute Task は切り出した個別作業をサブエージェントへ自律実行させる委託の仕組みだ 。思考（設計）と実装（破壊的変更）を明確に分離し、コンテキスト肥大化を防ぎながら安全にコーディングを進める構造になっている 。

## 各機能の役割と仕組み

### Plan
Plan は、ファイル変更やコマンド実行を原則制限された読み取り専用の計画エージェントだ 。コードベースの検索や構造解析を行い、実際の実装手順や設計方針を練る目的で使う 。プロジェクトのソースコードを勝手に書き換えないため、影響範囲の洗い出しや方針のすり合わせを安全に行える 。

### Build
Build は、ファイル作成・編集・削除やシェルコマンド実行の権限を持つ標準の実装エージェントだ 。Plan で固めた設計をもとに、実際のコード修正、テスト実行、リファクタリングを直接実行する 。ターミナルUIでは `Tab` キーひとつで Plan と Build を交互に切り替えられる 。

### Execute Task
Execute Task（内部ツールの `task` 呼び出し）は、特定の作業単位を切り出して専門のサブエージェントに独立実行させる委託処理だ 。親セッションから子セッションを生成してタスクを走らせ、完了後に結果のみを親コンテキストへ還元する 。これによりメインのトークン履歴を汚さず、調査や局所的な検証を並行・自律処理できる 。

## 機能比較

| 項目 | Plan | Build | Execute Task |
|---|---|---|---|
| 主な目的 | コード解析と実装計画の策定  | コードの記述・編集と検証  | サブエージェントへの個別作業委託  |
| ファイル変更 | 不可（計画用ファイル等を除く）  | 可能（フルアクセス）  | 委託先サブエージェントの権限に準拠  |
| シェル実行 | 原則禁止または確認要求  | 実行可能  | 委託先サブエージェントの権限に準拠  |
| 動作単位 | プライマリエージェント  | プライマリエージェント  | サブセッション / ツール呼び出し  |

## 基本の作業サイクル

1. Plan モードで要件を伝え、変更対象ファイルと作業ステップを計画させる 。
2. 計画の妥当性を確認したら、`Tab` で Build モードへ移行する 。
3. Build エージェントが実装を進め、調査や並行作業が必要な局面で Execute Task を介してサブエージェントを動かす 。
4. 実装完了後にテストを実行し、意図通りの動作を確認して終了する 。

##
