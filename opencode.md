...
- 作業記録を書くときは、記録ファイルへ現在時刻を含んだタイトルで日記形式で追加書き込みすること。
- 現在時刻をファイル名に含めるときは以下の形式を使うこと。
  - date +%Y_%m%d_%H%M_%S

- このプロジェクトを解析したい。そのために、まずドキュメントファイルとソースコードファイルに何があるか一覧を探して、./.work/docs_and_codes_<現在時刻>.md というファイル名に記録すること本文中にも現在時刻を記録すること。
